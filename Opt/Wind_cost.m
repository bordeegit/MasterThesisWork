function [ftot, gradtot] = Wind_cost(z,p)
    ...
    if nargout > 1 % gradient required
        ...
        gradF(1,3) = -sqrt(p.Q(3,3))/m * (Flconst*zl_z*2*wa(1) + ...
            Fdconst*2*wa(3)*wa(1)/(2*wa_norm));
        gradF(2,3) = -sqrt(p.Q(3,3))/m * (Flconst*zl_z*2*wa(2) + ...
            Fdconst*2*wa(3)*wa(2)/(2*wa_norm));
        gradF(3,3) = sqrt(p.Q(3,3))/m * Flconst*wa_norm^2*2*z(3);
        gradF(4,3) = sqrt(p.Q(3,3))/m * Flconst*wa_norm^2*2*z(4);
