function [ftot,gradtot] = Wind_cost(z,p)
    ...
    if nargout > 1 % gradient required
        ...
        gradF(2,1) = -sqrt(p.Q(1,1))/m * (Flconst*z(4)*2*wa(2) + ...
            Fdconst*2*wa(1)*wa(2)/(2*wa_norm));
        gradF(3,1) = -sqrt(p.Q(1,1))/m * (Flconst*z(4)*2*wa(3) + ...
            Fdconst*2*wa(1)*wa(3)/(2*wa_norm));
        gradF(4,1) = -sqrt(p.Q(1,1))/m * Flconst*wa_norm^2;

        gradF(1,2) = -sqrt(p.Q(2,2))/m * (Flconst*z(5)*2*wa(1) + ...
            Fdconst*2*wa(2)*wa(1)/(2*wa_norm));
        gradF(2,2) = -sqrt(p.Q(2,2))/m * (Flconst*z(5)*2*wa(2) + ...
            Fdconst*wa_norm + Fdconst*2*wa(2)^2/(2*wa_norm));
        gradF(3,2) = -sqrt(p.Q(2,2))/m * (Flconst*z(5)*2*wa(3) + ...
            Fdconst*2*wa(2)*wa(3)/(2*wa_norm));
        gradF(5,2) = -sqrt(p.Q(2,2))/m * Flconst*wa_norm^2;

        gradF(1,3) = -sqrt(p.Q(3,3))/m * (Flconst*z(6)*2*wa(1) + ...
            Fdconst*2*wa(3)*wa(1)/(2*wa_norm));
        gradF(2,3) = -sqrt(p.Q(3,3))/m * (Flconst*z(6)*2*wa(2) + ...
            Fdconst*2*wa(3)*wa(2)/(2*wa_norm));
        gradF(3,3) = -sqrt(p.Q(3,3))/m * (Flconst*z(6)*2*wa(3) + ...
            Fdconst*wa_norm + Fdconst*2*wa(3)^2/(2*wa_norm));
        gradF(6,3) = -sqrt(p.Q(3,3))/m * Flconst*wa_norm^2;
